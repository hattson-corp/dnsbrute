...
class dnsbrute:
    def __init__(self, site=None, wordlist=None, silent=0, workdir=''):
        self.silent = silent
        self.site = site
...
parser.add_argument('-p', '--workpath', help="path of curent directory , you can use $(pwd) instead ", required=True)
arg = parser.parse_args()
site = arg.domain
workdir = arg.workpath
wordlist = arg.wordlist
silent = int(arg.silent)
app = dnsbrute(wordlist=wordlist, site=site, silent=silent, workdir=workdir)
app.run()
